...
def preprocess_text(text):
    # Simple sentence splitter using regex (splits on ., !, or ? followed by space or end of string)
    sentences = re.split(r'(?<=[.!?])\s+', text.strip())
    words = [word.lower() for sentence in sentences for word in re.findall(r'\b\w+\b', sentence)]
